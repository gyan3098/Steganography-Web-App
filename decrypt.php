<?php
session_start();
if($_SESSION['fileDest'] != ''){
include('functions.php');

$src = $_SESSION['fileDest']; //Change this to the image to decrypt
...
for ($x = 0; $x < ($width*$height); $x++) { //Loop through pixel by pixel
  ...
  if ($count == 8) { //Every time we hit 8 new digits, check the value
      if (toString(substr($real_message, -8)) === '|') { //Whats the value of the last 8 digits?
          ...
          $_SESSION['message'] = $real_message;
          echo $_SESSION['message'];
          header("Location: imgupload2.php?mess2='successfully_decrypted'");
          die;
      }
