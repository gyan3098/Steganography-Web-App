<?php
session_start();
if($_SESSION['message'] != '' && $_SESSION['fileDest'] != ''){
...
echo '<br>';
$msg = $_SESSION['message']; //To encrypt
$src = $_SESSION['fileDest']; //Start image
echo $src."<br>";
...
$crypted= ('result' . $randomDigit . '.png'); //Echo our image file name
echo $crypted;
imagedestroy($img); //get rid of it


if(error_get_last() === true){
  header("Location: imageupload.php?mess='unable to encrypt'");
}
$_SESSION['result'] = $crypted;
header("Location: imageupload.php?mess='successfully_encrypted'");
}
